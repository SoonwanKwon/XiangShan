...
This document is a clearer, phase-1 view of the XiangShan frontend. It focuses on
how instructions flow from prediction to fetch to decode, and how redirects and
updates travel back. The goal is to make the overall functional flow easy to
understand before diving into module internals.
...
2. **FTQ → IFU**: Fetch requests carry PC and prediction metadata to IFU.
3. **IFU ↔ ICache**: Cacheline requests and responses provide instruction bytes.
4. **IFU → IBuffer**: Predecoded instructions and metadata are buffered for decode.
5. **Backend → FTQ → BPU**: Redirects and updates recover from mispredictions and train predictors.
...
## 9. Overall Frontend Functional Flow (Clear Walkthrough)

This section explains the end-to-end frontend flow in a single, linear story.
Think of it as: **Predict -> Queue -> Fetch -> Predecode -> Buffer -> Decode -> Update**.

### 9.1 One-Line Mental Model

**BPU makes a guess -> FTQ remembers it -> IFU fetches bytes -> IFU predecodes ->
IBuffer queues instructions -> Backend decodes/executes -> FTQ/BPU get corrections.**

### 9.2 Step-by-Step Flow (Normal Case)

1. **BPU predicts the next fetch block.**
   - Inputs: current PC and global history.
   - Outputs: predicted targets, taken bits, CFI info, and meta.
2. **FTQ enqueues the prediction.**
   - Stores PC, target, prediction bits, and meta in an FTQ entry.
   - Provides a stable "fetch context" for later stages.
3. **FTQ issues a fetch request to IFU.**
   - `fromFtq.req` carries start PC, FTQ index, and prediction context.
4. **IFU F0 sends requests to ICache and ITLB.**
   - ICache: fetch the cache line containing the start PC.
   - ITLB: translate and check execute permissions.
5. **IFU F1 calculates per-slot PCs.**
   - For `PredictWidth` slots, compute `pc[i]`.
   - Handle cross-line alignment.
6. **IFU F2 receives cacheline and predecodes.**
   - Extract up to 16 instructions from the cacheline.
   - Predecode identifies RVC/RVI, branch/JAL/JALR, and boundary cut points.
   - Check ITLB exceptions.
7. **IFU F3 verifies prediction and handles MMIO.**
   - If predicted CFI disagrees with predecode, prepare redirect.
   - If MMIO, enter MMIO path.
8. **WB writes results back.**
   - `toFtq.pdWb` returns predecode data to FTQ (for later verification).
   - `toIbuffer` sends instructions + metadata to IBuffer.
9. **IBuffer feeds Backend decode.**
   - Backend consumes instruction stream in order.
10. **Backend commits or redirects.**
   - On correct prediction: commit and train BPU.
   - On misprediction: redirect (flush) and restart from correct target.

### 9.3 Redirect and Recovery Paths (When Things Go Wrong)

There are three main redirect sources, in priority order:

1. **Backend redirect (highest priority).**
   - Real execution outcome disagrees with prediction.
   - FTQ resets pointers, BPU restores history, IFU/IBuffer flushed.
2. **IFU WB redirect.**
   - IFU predecode disagrees with BPU prediction (wrong target, CFI type).
   - Flushes younger fetch stages and restarts from corrected target.
3. **BPU S2/S3 redirect (early correction).**
   - Later predictor stages override the earlier prediction (S1 -> S2/S3).
   - FTQ/IFU flush younger stages and re-fetch from new target.

### 9.4 Where Each Module Fits (Big Picture Map)

| Module | Primary Role | Inputs | Outputs |
|--------|--------------|--------|---------|
| **BPU** | Predict next fetch path | PC, global history | Predictions to FTQ |
| **FTQ** | Buffer and track fetch context | BPU predictions, backend redirects | Fetch requests, updates |
| **IFU** | Fetch and predecode | FTQ requests, ICache line | Instructions to IBuffer |
| **ICache/ITLB** | Provide instruction bytes & translation | IFU request | Cacheline, exceptions |
| **IBuffer** | Smooth delivery to backend | IFU predecoded instrs | Decode stream |
| **Backend** | Execute + verify | IBuffer stream | Redirects, update info |

### 9.5 Key Signals in the Flow (Short Glossary)

- **`bpu_to_ftq.resp`**: predictor results pushed into FTQ.
- **`fromFtq.req`**: FTQ issues a fetch request to IFU.
- **`toICache.req`**: IFU requests an instruction cache line.
- **`icacheInter.resp`**: cache line data returned to IFU.
- **`toIbuffer`**: IFU sends predecoded instructions and metadata.
- **`redirect` (Backend or IFU)**: flush and restart fetch from correct target.
- **`ftq_to_bpu.update`**: training data for predictors.

### 9.6 A Minimal Timing View (5-cycle mental picture)

This is not exact cycle timing, but a simple mental model:

```
Cycle N:    BPU predicts, FTQ enqueues, IFU F0 issues fetch
Cycle N+1:  IFU F1 computes PCs
Cycle N+2:  IFU F2 gets cacheline and predecodes
Cycle N+3:  IFU F3 verifies, prepares redirect if needed
Cycle N+4:  WB sends to FTQ and IBuffer
```

If a redirect happens, the pipeline flushes younger stages and restarts from the
correct target; the BPU history is also repaired using FTQ/Backend info.
