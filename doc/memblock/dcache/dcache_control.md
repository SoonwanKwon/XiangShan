...
**Probe**는 캐시 일관성(Cache Coherence) 프로토콜에서 L2 캐시가 L1 캐시로 보내는 요청입니다.

멀티코어 시스템에서 여러 코어가 같은 메모리 주소를 캐시할 수 있습니다. 한 코어가 데이터를 수정하면 다른 코어의 캐시는 stale(오래된) 상태가 됩니다. 이를 방지하기 위해 L2가 L1에게 "권한을 내려놓으라"고 요청하는 것이 Probe입니다.

```
Core A의 L1 캐시: [주소 X] = 100 (Exclusive 상태)
Core B가 주소 X를 읽으려 함
     │
     ▼
L2 Cache가 Core A에게 Probe 전송
"Core A야, 주소 X에 대한 권한을 내려놔"
     │
     ▼
Core A는 ProbeAck 응답 (dirty면 데이터도 함께)
상태: Exclusive → Shared 또는 Invalid
```

TileLink Probe 파라미터:
- `toN (toNothing)`: 캐시 라인 무효화 (Invalidate)
- `toB (toBranch)`: 권한 낮추기 (Exclusive/Modified → Shared)
- `toT (toTrunk)`: 권한 유지하되 확인

#### AMO (Atomic Memory Operation)

**AMO**는 원자적 메모리 연산으로, 여러 코어가 동시에 같은 메모리를 접근할 때 race condition 없이 안전하게 read-modify-write를 수행합니다.

RISC-V AMO 명령어:
- `AMOSWAP`: 값 교환 (swap)
- `AMOADD`: 원자적 덧셈
- `AMOAND/AMOOR/AMOXOR`: 원자적 비트 연산
- `AMOMIN/AMOMAX`: 원자적 최솟값/최댓값
- `LR/SC`: Load-Reserved / Store-Conditional (CAS 구현용)

AMO는 메모리까지 가면 너무 느리기 때문에 L1 캐시에서 직접 처리합니다. 해당 라인의 Exclusive/Modified 권한이 필요합니다.

#### Replace (캐시 교체)

**Replace**는 캐시가 가득 찼을 때 새 데이터를 위해 기존 라인을 쫓아내는(evict) 작업입니다.

```
Cache Miss 발생 (주소 Y 접근)
     │
     ▼
모든 way가 사용 중 (set associative cache에서)
     │
     ▼
Replacement Policy로 victim 선택 (예: LRU)
     │
     ├── Victim이 Clean: 그냥 덮어씀
     │
     └── Victim이 Dirty: WritebackQueue로 Release 전송
                         (L2에 dirty 데이터 내려보냄)
     │
     ▼
새 데이터로 refill
...
### 2.1 Enqueue/Dequeue 조건

| 항목 | 설명 |
|------|------|
| **Enqueue Trigger** | LoadPipe S2에서 Load Miss 발생 시<br>MainPipe S2에서 Store/AMO Miss 발생 시<br>Prefetch 요청 시 |
| **Enqueue 소스** | `LOAD_SOURCE(0)`, `STORE_SOURCE(1)`, `AMO_SOURCE(2)`, `DCACHE_PREFETCH_SOURCE(≥3)` |
| **Dequeue Trigger** | `release_entry = s_grantack && w_refill_resp && w_mainpipe_resp`<br>(GrantAck 발행 완료 + Refill 완료 + MainPipe 처리 완료) |
| **Entry 할당 Priority** | `PriorityEncoder(primary_ready)` - 가장 낮은 인덱스의 빈 entry 우선 |
| **TileLink A 접근** | Round-Robin Arbiter로 entry 간 공정 접근 |

```
Enqueue 흐름:
LoadPipe/MainPipe ──► MissQueue Arbiter ──► Alloc/Merge/Reject 판정
                            │
                            ▼
                     ┌──────────────┐
                     │ S0: 중재     │  primary_ready? → Alloc (새 entry)
                     │              │  secondary_ready? → Merge (기존 entry에 병합)
                     │              │  secondary_reject? → Reject (재시도 필요)
                     └──────┬───────┘
                            │ (1 cycle)
                            ▼
                     ┌──────────────┐
                     │ S1: 실제 할당│  entry에 요청 정보 기록
                     └──────────────┘
```
...
### 2.6 Miss 요청 처리 로직: Alloc vs Merge vs Reject
...
### 3.1 Enqueue/Dequeue 조건

| 항목 | 설명 |
|------|------|
| **Enqueue Trigger** | L2 Cache에서 TileLink B 채널로 Probe 요청 수신 시<br>`io.mem_probe.valid && allocate` |
| **Dequeue Trigger** | MainPipe에서 probe 처리 완료 응답 수신 시<br>`io.pipe_resp.valid && io.id === io.pipe_resp.bits.id` |
| **Entry 할당 Priority** | `PriorityEncoder(primary_ready)` - 가장 낮은 인덱스의 빈 entry 우선 |
| **MainPipe 접근** | Arbiter로 entry 간 경쟁, 선택된 요청은 1 cycle delay 후 MainPipe로 전송 |
| **특수 조건** | LR/SC로 잠긴 블록에 대한 Probe는 `lrsc_blocked`로 대기 |

```
Enqueue/Dequeue 흐름:
TileLink B ──► ProbeQueue Entry 할당 ──► MainPipe 요청 ──► 처리 완료 ──► Entry 해제
   │               (s_invalid)           (s_pipe_req)    (s_wait_resp)   (s_invalid)
   │                    │                     │               │
   │                    │                     │               │
   └── io.mem_probe ────┘                     └───────────────┘
                                                  pipe_resp
...
### 4.1 Enqueue/Dequeue 조건

| 항목 | 설명 |
|------|------|
| **Enqueue Trigger** | MainPipe S3에서:<br>• Probe 응답 시 (`voluntary=false`, ProbeAck)<br>• Miss 중 dirty 교체 시 (`voluntary=true`, `delay_release=true`)<br>• Replace 요청 시 (`voluntary=true`, `delay_release=true`) |
| **Dequeue Trigger** | **ProbeAck**: `release_done` (모든 beat 전송 완료) 즉시 해제<br>**Release**: `ReleaseAck 수신` 후 해제 |
| **Entry 할당 Priority** | `PriorityEncoder(primary_ready_vec)` |
| **TileLink C 접근** | Round-Robin Arbiter (`TLArbiter.robin`)로 entry 간 공정 접근 |
| **특수 조건** | `delay_release=true`면 `s_sleep` 상태에서 RefillPipe의 `release_wakeup` 대기 |

```
상태 전이 및 Dequeue 조건:

[ProbeAck 경로] - 즉시 전송
s_invalid ──(alloc && !delay_release)──► s_release_req ──(release_done)──► s_invalid
                                              │
                                              └── TileLink C로 ProbeAck 전송

[Release 경로] - Refill 완료 대기 후 전송
s_invalid ──(alloc && delay_release)──► s_sleep ──(release_wakeup)──► s_release_req
                                            │                              │
                                            │                              ▼
                                            │                        s_release_resp
                                            │                              │
...
### 5.2 요청 Priority 및 Arbiter

MainPipe는 여러 소스에서 요청을 받으며, **고정 우선순위(Fixed Priority)**로 중재합니다:

```
우선순위 (높음 → 낮음):
┌────────────────────────────────────────────────────────────────────┐
│  1. Probe (최우선)    ← ProbeQueue에서, 캐시 일관성 유지 필수      │
│  2. Replace           ← MissQueue에서, miss 처리 진행 위해 필요   │
│  3. Store             ← SBuffer에서, storeCanAccept 조건 만족 시  │
│  4. Atomic (최하위)   ← AtomicsUnit에서                           │
└────────────────────────────────────────────────────────────────────┘

arbiter(
  in = Seq(io.probe_req, io.replace_req, store_req, io.atomic_req),
  out = req,
  name = Some("main_pipe_req")
)
```

**Store 수락 조건** (`storeCanAccept`):
```scala
val storeCanAccept = storeWaitTooLong ||  // Store가 너무 오래 대기 (threshold 초과)
                     !loadsAreComing ||   // Load 요청이 없음
                     io.force_write       // 강제 쓰기 신호
```

### 5.3 4-Stage Pipeline 구조
